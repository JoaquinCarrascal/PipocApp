...
export class MovieServService { 
  ...
  getTopRated(): Observable<TopRatedResponse>{
    ...
    return this.http.get<TopRatedResponse>(`${BASE_URL}?include_adult=false&include_video=false&language=en-US&page=1&sort_by=vote_count.desc&with_watch_monetization_types=free`, {
      headers: {
  ...
  getPopular(): Observable<TopRatedResponse>{
    ...
    return this.http.get<TopRatedResponse>(`${BASE_URL}?include_adult=false&include_video=false&language=en-US&page=1&primary_release_year=${thisYear}&sort_by=popularity.desc&vote_average.gte=8&vote_count.gte=100&api_key=${environment.API_KEY}`);
  ...
  getPopularTv(): Observable<TvTopRatedResponse>{
    ...
    return this.http.get<TvTopRatedResponse>(`${BASE_URL_TV}?first_air_date_year=${thisYear}&include_adult=false&include_null_first_air_dates=false&language=en-US&page=1&sort_by=popularity.desc&vote_average.gte=7&vote_count.gte=100&api_key=${environment.API_KEY}`);
  
  }

  getPopularTvByGenre(genreId: number): Observable<TvTopRatedResponse>{

    return this.http.get<TvTopRatedResponse>(`${BASE_URL_TV}?include_adult=false&include_null_first_air_dates=false&language=en-US&page=1&sort_by=popularity.desc&with_genres=${genreId}&api_key=${environment.API_KEY}`);
